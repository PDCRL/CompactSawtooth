...
def do_deposit(args):
    ...
    transaction.ParseFromString(batch_bytes)
...
def do_transfer(args):
    ...
    transaction=Transaction()
    batch_bytes = clientFrom.withdraw(args.value)
    transaction.ParseFromString(batch_bytes)
    transaction_list.append(transaction)
...
def main(prog_name=os.path.basename(sys.argv[0]), args=None):
    ...
    clientFrom = SimpleWalletClient(baseUrl=DEFAULT_URL,keyFile=keyfile)

    response = clientFrom._send(transaction_list)
    print("Response: {}".format(response))
